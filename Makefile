...
install-pydantic_ssm_settings:
	python -m pip install -U wheel pip
	pip install -r requirements.txt
	SKIP_CYTHON=1 pip install -e .

.PHONY: install-testing
install-testing: install-pydantic_ssm_settings
	pip install -r tests/requirements-testing.txt

# .PHONY: install-docs
# install-docs: install-pydantic_ssm_settings
# 	pip install -U -r docs/requirements.txt

# .PHONY: install-benchmarks
# install-benchmarks: install-pydantic_ssm_settings
